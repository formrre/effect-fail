...
These lemmas apply only to the free monad with a forgetful functor $Mnd(C) \rightarrow Endo(C)$, we suggest a generalization that should extend these to the forgetful functor $Mnd(C) \rightarrow (C,ob(C))$.

There is no obvious generalization, which would let us talk about algebras over an arbitrary $(C,ob(C))$, we thus take a type theoretic view and think what would we expect the isomorphism to involve.
Semantics of a type family $G$ is thought as an initial object in some category $IAlg(G)$ corresponding to its algebraic description.
Homotopy type theory generalizes this to quotients (and other more general higher inductive types).
Every endofunctor also has an associated category of algebras, we propose that this category for left Kan extension of G, $Lan(G)$, considered as an endofunctor should
be isomorphic to the category $IAlg(G)$.

This should allow us to extend the isomorphism in the definition of algebraic freeness to the freer construction. Thus the following conjecture.
...
We would expect the formal statement of this conjecture to be provable in some appropriate container \cite{indexed containers} or generic levitated presentation \cite{levitation},
which would allow us to connect the \mathbb{Lan F}-algebra of an endofunctor with the algebra of the underlying datatype \mathbb{F}.
